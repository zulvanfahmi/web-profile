    ...
    return (
        <div className='selection:bg-fuchsia-300 selection:text-fuchsia-900'>
            ...
            <div className='bg-sky-900 dark:bg-zinc-800 border-t-2 border-sky-800 dark:border-zinc-700 sticky py-4 bottom-0 sm:hidden text-white'>
                <div className={`flex flex-row ${isBottomNavbarOpen ? 'items-start' : 'items-center'} mx-4 gap-4`}>
                    <ul className='grow flex flex-col gap-1'>
                        {
                            pageSections.map((pageSection, index) => (

                                <li key={index} className={`my-0.5 tracking-wider ${pageSection.idPage === activeSection ? styleActiveSection : styleInactiveSection}`}>

                                    <a href={`#${pageSection.idPage}`} className="flex items-center space-x-2">
                    ...
                    <div className='flex items-center gap-4'>
                        <button onClick={() => setIsBottomNavbarOpen(!isBottomNavbarOpen)} className="transition duration-500 ease-in-out">
                            <IconContext.Provider value={{ className: `text-3xl transition-transform duration-600 ${isBottomNavbarOpen ? 'rotate-180' : 'rotate-0'}` }}>
                                <IoIosArrowUp />
